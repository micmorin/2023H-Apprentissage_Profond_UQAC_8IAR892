...
# Board (Obj) [22]
## init(level) [27]
## initialMatrix(level) [30]
### verifyRows() [50]
### verifyColumns() [72]
### addObstructions(disruption_matrix) [94]
## enactMove(move, current_level) [111]
### adjustDisruptions(reduce_timer) [136]
### calculateScore(initial_board, current_level) [166]
## verifyMove(move) [172]
### verifyOneRow(row, column, value)  [190]
### verifyOneColumn(row, column, value) [202]
## fall() [214]
## printLayout(current_level) [238]
########

class Board(object):

    layout = np.zeros([6, 6], dtype = int)
    barriers = np.zeros([6, 6], dtype = int)
    ...
    def enactMove(self, args, current_level):
        ...
        while np.where(self.layout == 0)[0].size != 0:
            self.calculateScore(copy, current_level)
            self.fall()
    ...
    def calculateScore(self, initial_board, current_level):
        ...
        for i in removed: 
            current_level.score += int(current_level.team[i%10][1]) # Use the extracted values to get the pokemon's power and add to score
